...
function displayMovie() {
	...
	for (let i = 0; i < moviesArr.length; i++) {
		...
		movieElement.innerHTML = `
            ...
            <div class="card-body">
                <h4 class="card-title fw-bold">${moviesArr[i].title} [${moviesArr[i].year}]</h4>
                <h5 class="fw-bold text-danger">${moviesArr[i].ticketPrice} ৳</h5>
                <button class="btn btn-dark fw-bold bookTicket">
                    Book Ticket
                </button>
            </div>
        `;
...
function displayModal() {
	const bookTicket = document.querySelectorAll(".bookTicket");
			...
			for (let j = 0; j < moviesArr.length; j++) {
				if (i == j) {
					...
					modalWindow.innerHTML = `
                        ...
                        <button class="btn btn-danger fw-bold mt-4 buyNow">Buy Now</button>
                    `;
					...
					const closeBuyBtn = document.querySelector(".closeBuyBtn");
